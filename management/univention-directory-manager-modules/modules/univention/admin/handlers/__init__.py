...
class simpleLdap(object):
	...
	def __init__(self, co, lo, position, dn=u'', superordinate=None, attributes=None):
		...
		if s4connector_present is None:
			...
			searchResult = self.lo.search(u'(&(|(objectClass=univentionDomainController)(objectClass=univentionMemberServer))(univentionService=S4 Connector))', attr=['aRecord', 'aAAARecord'])
			s4connector_present = any(ddn for (ddn, attr) in searchResult if set([u'aAAARecord', u'aRecord']) & set(attr))
	...
	def _move_in_subordinates(self, olddn):
		result = self.lo.search(base=self.lo.base, filter=filter_format(u'(&(objectclass=person)(secretary=%s))', [olddn]), attr=['dn'])
			...
			try:
				...
				if result:
					for dn, attr in result:
						if 'aRecord' in attr:
							zoneNames.append((attr['zoneName'][0].decode('UTF-8'), [x.decode('ASCII') for x in attr['aRecord']]))
						if 'aAAARecord' in attr:
							zoneNames.append((attr['zoneName'][0].decode('UTF-8'), map(lambda x: ipaddr.IPv6Address(x).exploded, [x.decode('ASCII') for x in attr['aAAARecord']])))
				...
				for zoneName in zoneNames:
					...
					try:
						...
						for dn, attr in results:
							ip = self.__ip_from_ptr(attr['zoneName'][0].decode('UTF-8'), attr['relativeDomainName'][0].decode('UTF-8'))
			...
			for dn, attributes in self.lo.search(scope='domain', attr=['pTRRecord'], filter=filter_format('(&(relativeDomainName=%s)(zoneName=%s))', (rdn, zone_name))):
				if len(attributes['pTRRecord']) == 1:
					...
					for dn2, attributes2 in self.lo.search(scope='domain', attr=['zoneName'], filter=filter_format('(&(relativeDomainName=%s)(objectClass=dNSZone))', [name]), unique=False):
						self.lo.modify(dn, [('pTRRecord', b'%s.%s.' % (name.encode('UTF-8'), attributes2['zoneName'][0]), b'')])
		...
		if name and zoneDn and ip:
			...
			if not results:
				self.lo.add('relativeDomainName=%s,%s' % (escape_dn_chars(ipPart), zoneDn), [
					...
					('zoneName', [explode_dn(zoneDn, 1)[0].encode('UTF-8')]),
	...
	def __modify_dns_forward_object(self, name, zoneDn, new_ip, old_ip):
		...
		if old_ip and new_ip:
			...
			for dn, attr in results:
				...
				if ':' in new_ip:  # IPv6
					new_ip = ipaddr.IPv6Address(new_ip).exploded.encode('ASCII')
					if new_ip not in new_aAAARecord:
						new_aAAARecord.append(new_ip.encode('ASCII'))
				else:
					if new_ip not in new_aRecord:
						new_aRecord.append(new_ip.encode('ASCII'))
				modlist = []
			...
			if not results:
				...
				for dn, attr in results:
					if 'aAAARecord' in attr:
						...
						if ip.encode('ASCII') not in new_ip_list:
							new_ip_list.append(ip.encode('ASCII'))
			...
			if not results:
				...
				for dn, attr in results:
					if 'aRecord' in attr:
						...
						self.lo.modify(dn, [('aRecord', b'', ip.encode('ASCII'))])
