...
class mapping(object):
	...
	def mapValue(self, map_name, value):
		...
		kwargs = {}
		if map_value and 'encoding' in inspect.getargspec(map_value).args:
			kwargs['encoding'] = (encoding, strictness)

		if six.PY3 and not map_value:
			if map_name == 'MRAcceptRecurringItems':
				asdf
	...
	def unmapValue(self, unmap_name, value):
		...
		kwargs = {}
		if unmap_value and 'encoding' in inspect.getargspec(unmap_value).args:
			kwargs['encoding'] = (encoding, strictness)
